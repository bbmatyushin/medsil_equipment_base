...
class EquipmentAdmin(MainModelAdmin):
    ...
    fieldsets = (
        (
            "Новое оборудование",
            {"fields": ("full_name", "short_name", "med_direction")},
        ),
        (
...
class EquipmentAccountingAdmin(MainModelAdmin):
    ...
    actions = MainModelAdmin.actions + [
        "set_is_our_service",
    ]
    ...
    search_fields = (
        ...
        "equipment__short_name",
        "serial_number",
        "equipment_acc_department_equipment_accounting__department__name",
    ...
    search_help_text = (
        "Поиск по полному и краткому наименованию оборудования, по его серийному номеру или "
    ...
    fieldsets = (
        (
            "НОВОЕ ОБОРУДОВАНИЕ ДЛЯ УЧЁТА",
            {
                "fields": (
                    "equipment",
                    ("serial_number", "equipment_status"),
                    (
                        "is_our_supply",
                        "is_our_reagents",
                    ),
    ...
    def get_search_results(self, request, queryset, search_term):
        ...
        for term in search_terms:
            ...
            equipment_query = (
                Q(equipment__full_name__icontains=term)
    ...
    def get_queryset(self, request):
        ...
        user_position = request.user.position.filter(
            type="employee", name__iexact="менеджер"
        )
        if user_position:
        ...
        queryset = queryset.select_related(
            ...
            Prefetch(
                ...
                queryset=Service.objects.select_related("service_type"),
            ),
        )
        ...
        queryset = queryset.order_by(
            "-active_install_dt", "equipment", "serial_number", "user"
        )
...
class ServiceAdmin(MainModelAdmin):
    ...
    def get_queryset(self, request):
        ...
        return (
            ...
            .prefetch_related(
                ...
                Prefetch("service_photos", queryset=ServicePhotos.objects.all()),
                Prefetch(
    ...
    def job_content_short(self, obj):
        ...
        content = (
            "-"
            if not job_content
            else job_content if len(job_content) < 50 else f"{job_content[:50]}..."
        )
        return (
            f"{content}"
            if len(content) < 50
    ...
    def description_short(self, obj):
        ...
        descr = (
            "-"
            if not description
            else description if len(description) < 50 else f"{description[:50]}..."
        )
        return (
            f"{descr}"
            if len(descr) < 50
    ...
    def service_akt_url(self, obj):
        if obj.service_akt:
            ...
            return self._akt_mark_safe(
                url=url, akt_name=akt_name, tag_id="akt-create-btn", action="update"
            )
        if obj.pk:
    ...
    def accept_from_akt_url(self, obj):
        if obj.accept_from_akt:
            ...
            return self._akt_mark_safe(
                url=url,
                akt_name=akt_name,
                tag_id="accept-akt-from-create-btn",
                action="update",
            )
        if obj.pk:
            return self._akt_mark_safe(
                tag_id="accept-akt-from-create-btn", action="create"
            )
        # При создании новой записи направит сюда
        return self._akt_mark_safe()
    ...
    def _akt_mark_safe(
        ...
        if action == "create":
            return mark_safe(
                f'<span class="akt-span">Акт не создан</span>'
                f'<input type="button" id="{tag_id}" value="Создать">'
            )
        elif action == "update":
            time_version = int(time.time())
            return mark_safe(
                f'<span class="akt-span"><a href="{url}?v={time_version}">{akt_name}</a></span>'
    ...
    def get_equipment_ids(search_str: str) -> QuerySet:
        ...
        eq_acc = EquipmentAccounting.objects.filter(
    ...
    def get_fieldsets(self, request, obj=None):
        ...
        if (
            obj is not None
        ):  # для страницы редактирования убираем поля для поиска оборудования
        ...
        if obj and obj.equipment_accounting:
            ...
            if active_department and active_department.department:
                ...
                contact_persons = DeptContactPers.objects.filter(
                    department=active_department.department, is_active=True
                ...
                if (
                    ...
                    and "contact_person_id" in obj.contact_person_data
                ):
                    try:
                ...
                form.base_fields["contact_person"].queryset = (
                    DeptContactPers.objects.none()
                )
        else:
    ...
    def get_changeform_initial_data(self, request):
        ...
        eq_acc_id = request.GET.get(
            "choice_eq_acc"
        )  # выбранное оборудование из таблицы учета
        ...
        if db_field.name == "equipment_accounting":
            ...
            if re.search(r"\/service\/add\/", request.path):
                if eq_acc:
                    ...
                    eq_acc_queryset: QuerySet = EquipmentAccounting.objects.filter(
                        pk__in=eq_acc_ids
                    )
                else:
                    eq_acc_queryset: QuerySet = EquipmentAccounting.objects.all()

                kwargs["queryset"] = eq_acc_queryset.select_related(
                    "equipment",
                    "equipment__manufacturer",
                    "equipment__supplier",
                    "equipment__med_direction",
            ...
            used_replacement_equipment_ids = (
                Service.objects.filter(
            ...
            kwargs["queryset"] = (
                ReplacementEquipment.objects.select_related("equipment", "user")
    ...
    def formfield_for_manytomany(self, db_field, request, **kwargs):
        ...
        if db_field.name == "spare_part":
            ...
            if re.search(r"\/service\/add\/", request.path):
                if eq_acc_id:
                    eq_ids = EquipmentAccounting.objects.filter(
                        pk=eq_acc_id
            ...
            if eq_ids:
                ...
                kwargs["queryset"] = SparePart.objects.filter(
                    equipment__id__in=eq_ids
                ).prefetch_related(
                    "equipment",
                    "service",
                )
    ...
    def save_model(self, request, obj, form, change):
        ...
        if contact_person:
            ...
            obj.contact_person_data = {
                "fio": fio,
                "position": (
                    contact_person.position.name if contact_person.position else ""
                ),
                "department": (
                    contact_person.department.name if contact_person.department else ""
                ),
                "contact_person_id": str(
                    contact_person.id
                ),  # Сохраняем ID для доступа к телефонам
            }
...
class ReplacementEquipmentAdmin(MainModelAdmin):
    ...
    search_help_text = (
        "Поиск по модели оборудования, серийному номеру или подразделению"
    )
    # list_filter = ('state',)
    ...
    def related_service(self, obj):
        ...
        try:
            ...
            if service:
                ...
                return mark_safe(
                    f'<a href="{url}">{equipment.equipment.short_name}</a>'
                )
    ...
    def transferred_to(self, obj):
        ...
        try:
            ...
            if service and not service.returned_to_office:
                ...
                active_departments = (
                    service.equipment_accounting.equipment_acc_department_equipment_accounting.all()
                )
                departments = []
