    ...
    if created:
        ...
        if old_quantity > 0 and old_spare_part_id:
            try:
                ...
                SparePartCount.objects.filter(
                    spare_part=old_spare_part,
                    expiration_dt=old_exp_dt
                ).update(amount=Round(F('amount') + old_quantity, 2))
                logger.info(
                    'Returned old stock. Part: %s, Qty: %s, Exp: %s',
                    old_spare_part.name, old_quantity, old_exp_dt
                )
            except SparePart.DoesNotExist:
                logger.error('Old spare part (pk=%s) not found during update', old_spare_part_id)
